"""Bagging meta-estimator, modified for imbalanced problem domain.."""
 
# Author: Gilles Louppe <g.louppe@gmail.com>
# License: BSD 3 clause
#
# Adaptations by Rodrigo Santis <rodrigo.santis@live.com>.
#
...
def _parallel_build_estimators(n_estimators, ensemble, X, y, sample_weight,
    ...
    for i in range(n_estimators):
        ...
        # Resample each bag      
        if sampler == 'under':
            X_res, y_res = RandomUnderSampler(random_state=random_state
                                             ).fit_sample(X, y)
        elif sampler == 'over':
            X_res, y_res = RandomOverSampler(random_state=random_state
                                             ).fit_sample(X, y)
        elif sampler == 'smote':
            X_res, y_res = SMOTE(random_state=random_state, k_neighbors=5).fit_sample(X, y)
#        elif sampler == 'adasyn':
...
class BaseBagging(with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def fit(self, X, y, max_depth=None, sample_weight=None):
        ...
        self : object
            Returns self.
        """
        return self._fit(X, y, self.max_samples, self.max_depth, 
...
class Blagging(BaseBagging, ClassifierMixin):
    ...
    def _validate_estimator(self):
        ...
        super(Blagging, self)._validate_estimator(
            default=DecisionTreeClassifier())

    def _validate_sampler(self):
        """Check the estimator and set the base_estimator_ attribute."""
        super(Blagging, self)._validate_sampler(
